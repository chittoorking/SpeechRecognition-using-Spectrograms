...
#-------------------------------------------------------------------------
sample_rate, samples = wavfile.read(r'Train/audio/yes/7c1d8533_nohash_1.wav')
...
# Plotting the 30sec audio wave form
# Updated part of code
#------------------------------------------------------------
import wave
import sys

'''----------------Finding spectrogram data for 30sec Audio file--------------------'''
data = []

sample_rate, samples = wavfile.read(r'Testing_data.wav')
frequencies, times, spectrogram = signal.spectrogram(samples, sample_rate)
...
for file in sorted(glob.glob("*.wav")):
    print("#"*50)
    print(file)
    os.system("python pyAudioAnalysis/audioAnalysis.py silenceRemoval -i"+ file +" --smoothing 1.0 --weight 0.5")

data2 = []

audios = [file for file in sorted(glob.glob("Testing_data_*.wav"))]
for file in audios:
    # load the image, swap color channels, and resize it to be a fixed
    # 224x224 pixels while ignoring aspect ratio
    #file = audios[i]
    ...
    # update the data and labels lists, respectively
    if spectrogram.shape[0] >= 128 and spectrogram.shape[1] >=48:
        data2.append(spectrogram[:128,:48])

data2 = np.array(data2)
# make predictions on the testing set
predIdxs = model.predict(data2)
# for each image in the testing set we need to find the index of the
# label with corresponding largest predicted probability
predIdxs = np.argmax(predIdxs, axis=1)
for i in range(0,20):
    print(words[predIdxs[i]])
#------------------------------------------------------------
